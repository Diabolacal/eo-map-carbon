# New Eden catalogue artefacts

Slim static handoff for Milestones 1B and 1C. The Carbon executable does not
open SQLite, does not call ESI, and does not load the EO-Map web app.

## Files

- `new_eden_systems.bin` — little-endian `NEDEN1B` table of known-space id / name / Contract A XYZ
- `new_eden_systems.manifest.json` — provenance, AABB, and hub anchors
- `new_eden_stargates.bin` — little-endian `NEGATE1` table of unique undirected known-space pairs
- `new_eden_stargates.manifest.json` — provenance, filtering, and hub adjacency
- `new_eden_star_visuals.bin` — little-endian `NESTAR1` table of known-space id / `star_temperature` / spectral letter
- `new_eden_star_visuals.manifest.json` — provenance and temperature window
- `new_eden_regions.bin` — little-endian `NEREGN1` table of known-space id / Contract A `region_id`
- `new_eden_regions.manifest.json` — provenance and distinct-region count

## Provenance

Generated by `scripts/export-new-eden-systems.py` from the sibling EO-Map
Contract A universe database:

```
C:\dev\eo-map\eve-frontier-map\public\map_data_eo_3464040.db
```

That database is EO-Map's pinned SDE 3464040 artefact (builder 1.5.0). It is
the same file the live map parses in `universeDbParse.ts`. This export does not
re-read the SDE zip.

Pinned source SHA-256:

```
874262496556d933bbbc184cdec8f469660f6c1108b21e25f0481475b68eba81
```

Regenerate (requires the sibling EO-Map checkout):

```powershell
python .\scripts\export-new-eden-systems.py
```

## What is included

5,485 New Eden known-space systems (`id` 30000000–30999999, `hidden = 0`).

The Contract A table also contains 2,604 Anoikis / W-space systems. EO-Map
draws those too, but they sit in a separate cluster about 1,300 LY away and
are not part of the default New Eden framing. They are omitted here.

## Coordinates

The binary stores Contract A `position_x/y/z` unchanged (light years after
EO-Map's builder remap `(raw.x, raw.z, raw.y) / 9.46e15`).

The Carbon host applies EO-Map's live display mapping before the GPU:

```
scene = (db.x, -db.z, -db.y)
```

Vertices are not recentred. The camera target is the measured New Eden AABB
centre. There is no extra scale.

## Stargate graph

`new_eden_stargates.bin` is exported from the same Contract A `stargates`
table. That table already holds 13,978 directed known-space rows and zero
W-space / wormhole / jump-bridge edges. Carbon stores unique undirected pairs
(`source_id < dest_id`): **6,989** connections. Drawing both directed rows
would stack two identical 3D segments.

The binary stores system ids only. The host looks up the matching 1B scene
positions and emits a straight 3D segment. No 2D layout, curves, or midpoints.

## Star visuals

`new_eden_star_visuals.bin` is a sidecar, not a replacement of `NEDEN1B`.
It stores raw Contract A `star_temperature` (kelvin) and the first letter of
`star_class` for the same 5,485 ids, in the same order. The Carbon host derives
blackbody RGB and emissive at runtime (EO-Map `starColors.ts`) so sliders can
retune presentation. SDE radius / `luminosity_sde` / security status are not
exported; EO-Map's universe map does not use them for the default look.

## Regions

`new_eden_regions.bin` is a sidecar for optional Creator Mode tint. It stores
raw Contract A `region_id` for the same 5,485 ids, same order. Colour is
applied at runtime with EO-Map's 11-stop `REGION_ATLAS_PALETTE` (`region_id % 11`).
Default mix is off so stellar temperature colour stays intact.

Pinned checks derived from EO-Map, not invented:

- Jita region_id: 10000002 (The Forge); 70 distinct known-space regions
- Jita neighbours: Ikuchi, Maurasi, Muvolailen, New Caldari, Niyabainen, Perimeter, Sobaseki
  (`tools/eve-online-sde/test_contract_a.py`)
- Jita → Amarr = 11 hops
  (`eve-frontier-map/src/eo/routeDistance/__tests__/hopCount.test.ts`)
- Jita reachable set = 5,228 systems, including Jita
  (`generate-gate-topology.mjs` `mainComponent`)
- Niarja (30003504, Pochven) is present but unreachable from Jita
- Zarzakh's four static gates are included and drawn
