...
// Jita-Amarr shortest hop route: a second TOP_LINES buffer using the same GateLine
// program with a brighter tint, not the old grayscale three-draw host.
	...
	bool routeHighlight = false)
{
	...
	if (routeHighlight)
	{
		// Same GateLine program as the faded network. Higher opacity and a
		// lighter tint so the reconstructed Jita-Amarr path reads on top.
		data->gateParams[0] = 1.0f;
		data->gateTint[0] = 0.95f;
		data->gateTint[1] = 0.93f;
		data->gateTint[2] = 0.82f;
	}
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR cmdLine, int)
{
	...
	Log(stdout, "path: instanced star quads + TOP_LINES gates + TOP_LINES Jita-Amarr route + Creator Mode TrinityAL passes\n");
	...
	Log(stdout, "route: %s\n", FormatRouteNames(catalog, routeIds).c_str());
...
#include "StarFlare_ps.h"
	...
	if (Failed("Create route VB", routeVertexBuffer.Create(sizeof(StarVertex), routeVertexCount, Tr2GpuUsage::VERTEX_BUFFER, Tr2CpuUsage::NONE, routeLines.data(), *renderContext)))
	{
		CloseSmokeLog();
		return 1;
	}
	...
	Log(stdout, "Creator Mode: sky, ISM, glow/flare, region tint, persist. Jita-Amarr shortest route is a brighter GateLine overlay. F1 panel, F7 1px points, F8 save+copy, F9 baseline.\n");
	if (smoke)
	{
	...
	while (PumpMessages())
	{
		...
		if (!UpdateFrameConstants(frameCb, *renderContext, state, true) ||
			Failed("Set route VS CB", renderContext->SetConstants(frameCb, VERTEX_SHADER, 0)) ||
			Failed("Set route PS CB", renderContext->SetConstants(frameCb, PIXEL_SHADER, 0)) ||
			Failed("RS_ZWRITEENABLE route overlay", renderContext->SetRenderState(RS_ZWRITEENABLE, 0)) ||
			Failed("Set route stream", renderContext->SetStreamSource(0, routeVertexBuffer, 0, sizeof(StarVertex))) ||
			Failed("DrawPrimitive route", renderContext->DrawPrimitive(0, routeSegmentCount)) ||
			Failed("RS_ZWRITEENABLE restore", renderContext->SetRenderState(RS_ZWRITEENABLE, 1)) ||
			!UpdateFrameConstants(frameCb, *renderContext, state) ||
			Failed("Restore frame VS CB", renderContext->SetConstants(frameCb, VERTEX_SHADER, 0)) ||
			Failed("Restore frame PS CB", renderContext->SetConstants(frameCb, PIXEL_SHADER, 0)))
		{
			ok = false;
			break;
		}
		...
		if (fpsWindowMs >= 1000.0)
		{
			...
			Log(stdout, "perf: systems=%u connections=%u route_hops=%u draw_calls=%u pp_passes=%u bloom=%s stars=%s frame=%.2fms (%.0f fps) renderer=TrinityAL_DX11 dataset=SDE3464040\n",
				systemCount,
				edgeCount,
				routeHops,
				lastDraws,
	...
	if (smoke && frames >= kSmokeFrames && totalFrameMs > 0.0)
	{
		...
		Log(stdout, "smoke: frames=%u systems=%u connections=%u known_space=%u other_space=%u route_hops=%u bloom_on_draws=%u bloom_on_pp=%u bloom_off_creator_draws=%u bloom_off_creator_pp=%u creator_off_draws=%u creator_off_pp=%u star_draws=1 gate_draws=1 route_draws=1 bloom_on_frames=%u avg_frame_ms=%.2f avg_fps=%.1f path=TrinityAL_DX11/creator-mode dataset=SDE3464040\n",
			frames,
			systemCount,
			edgeCount,
			catalog.knownSpaceCount,
			...
			avgMs,
			fps);
		const uint32_t expectBloomOnDraws = startPoints ? 9u : 10u;
		const uint32_t expectBloomOffDraws = startPoints ? 6u : 7u;
		if (bloomOnDraws != expectBloomOnDraws || bloomOnPp != 6 || bloomOffCreatorDraws != expectBloomOffDraws ||
			bloomOffCreatorPp != 3 || lastDraws != 4 || lastPp != 1)
		{
			Log(stderr, "FAILED smoke draw/pass counts (expected %u/6, %u/3, 4/1)\n", expectBloomOnDraws, expectBloomOffDraws);
			ok = false;
		}
	}
